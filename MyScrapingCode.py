...
df = pd.DataFrame(columns=['Title', 'Price','Reviews', 'Store','URL','Page Number','Page Rank','Seller Status'])
...
for page_num in range(1, 6):
    ...
    for item in soup.find_all("a", {"class": "listing-link"}):
        ...
        try:
            ...
            ## Price
            for price in item.find_all('span', {"class": "currency-value"}):
                row['Price'] = price.get_text().strip()
            ## Reviews
